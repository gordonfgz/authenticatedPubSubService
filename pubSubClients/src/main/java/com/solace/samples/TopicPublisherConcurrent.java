 ...
 *
 */
public class TopicPublisherConcurrent {
	
	static boolean isShutdown = false;
    
    public void run(String... args) throws IOException {
        System.out.println("TopicPublisherConcurrent initializing...");
        ...
        try {
            ...
            try{
                for ( int i=0; i < num_threads; i++){
                    executor.execute(new PublisherRunnable(mqttClient, i, latch, num_msgs, 1000, 1000));                
                }
            }catch(Exception err){
                err.printStackTrace();
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length < 5) {
            System.out.println("Usage: topicPublisherConcurrent tcp://<host:port> [id_token] [access_token] [num_threads] [num_msgs per thread]");
            System.out.println();
            System.exit(-1);
        }
        new TopicPublisherConcurrent().run(args);
    }
}
