...
public class JsonPublisherRunnable implements Runnable{
    ...
    public JsonPublisherRunnable(MqttClient mqttClient, int i, CountDownLatch latch, int running_duration, int message_rate, int sleepTime){
    ...
    public void publishARandomMessage(int i) {
        try {
            ...
            // Publish the message
            mqttClient.publish("solace/json", message);
            ...
            System.out.println("Published json (count: " + (i+1) + ") from: " + Thread.currentThread().getName() + " @ " + formattedDateTime);
            try {
